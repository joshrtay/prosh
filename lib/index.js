...
function prosh (command, options = {}) {
  ...
  return findModules(cwd).then(node_modules_dir => {
    const env = assign({}, process.env, {PATH: `${process.env}:${join(node_modules_dir, 'node_modules/.bin')}`})

    return new Promise(function (resolve, reject) {
      let proc = spawn('/bin/sh', ['-c', command], {stdio: options.capture ? 'pipe' : 'inherit', cwd})
      let stdout = ''
      let stderr = ''
